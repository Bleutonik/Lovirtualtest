...
const Notes = () => {
  ...
  const loadNotes = async () => {
    ...
    try {
      ...
      const arr = res?.data ?? res?.notes ?? res ?? [];
      setNotes(Array.isArray(arr) ? arr : []);
    } catch (err) {
      flash(err?.message || 'Error al cargar las notas', 'error');
      setNotes([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingNote) {
        ...
        flash(t('notes.updated') || 'Nota actualizada');
      } else {
        await createNote(noteForm);
        flash(t('notes.created') || 'Nota creada');
      }
      closeModal();
      await loadNotes();
    } catch (err) {
      flash(err?.message || 'Error al guardar la nota', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteNote(id);
      flash(t('notes.deleted') || 'Nota eliminada');
      await loadNotes();
    } catch (err) {
  ...
  const fmtDate = (ds) => {
    try { return new Date(ds).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' }); }
    catch { return ''; }
  };

  const getColor = (id) => NOTE_COLORS[Math.abs(Number(id) || 0) % NOTE_COLORS.length];
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 stagger">
            {notes.map((note) => {
              ...
              return (
                <div key={note.id} className="rounded-2xl p-5 group animate-fade-up flex flex-col"
                  style={{ background: c.bg, border: `1px solid ${c.border}`, minHeight: '160px' }}>
                  ...
                  <p className="text-xs leading-relaxed flex-1 line-clamp-4" style={{ color: 'var(--text-muted)' }}>
                    {note.content || <span style={{ fontStyle: 'italic', opacity: 0.5 }}>{t('notes.noContent')}</span>}
                  </p>
        ...
        <div className="modal-backdrop" onClick={(e) => e.target === e.currentTarget && closeModal()}>
          <div className="modal p-6" style={{ maxWidth: '520px' }}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeModal} className="btn btn-ghost p-2" disabled={isSaving}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={closeModal} className="btn btn-ghost flex-1" disabled={isSaving}>
                  {t('common.cancel')}
                </button>
                <button type="submit" className="btn btn-primary flex-1" disabled={isSaving}>
                  {isSaving
                    ...
                    : editingNote ? t('common.save') : t('notes.new')
                  }
                </button>
              </div>
            </form>
